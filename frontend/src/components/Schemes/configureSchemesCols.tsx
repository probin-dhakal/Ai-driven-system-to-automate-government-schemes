...
export const loadColumns = async (
  ...
  const hasUpdatePermission = hasPermission(user, 'UPDATE_SCHEMES');
      ...
      getActions: (params: GridRowParams) => {
        return [
          <ListActionsPopover
            ...
            pathEdit={`/schemes/schemes-edit/?id=${params?.row?.id}`}
            pathView={`/schemes/schemes-view/?id=${params?.row?.id}`}
            key={1}
