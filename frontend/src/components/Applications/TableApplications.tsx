...
import { loadColumns } from './configureApplicationsCols';
...
const TableSampleApplications = ({
  ...
  useEffect(() => {
    if (applicationsNotify.showNotification) {
      notify(
        applicationsNotify.typeNotification,
        applicationsNotify.textNotification,
      );
    }
  }, [applicationsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/applications/applications-view/?id=${id}`);
  };
...
export default TableSampleApplications;
