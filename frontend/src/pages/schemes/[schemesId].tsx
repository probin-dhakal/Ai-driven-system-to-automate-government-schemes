...
const EditSchemes = () => {
  ...
  const { schemesId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: schemesId }));
  }, [schemesId]);
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: schemesId, data }));
...
EditSchemes.getLayout = function getLayout(page: ReactElement) {
  return (
