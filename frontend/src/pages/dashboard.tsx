...
const Dashboard = () => {
  ...
  const [applications, setApplications] = React.useState('Loading...');
  const [notifications, setNotifications] = React.useState('Loading...');
  const [schemes, setSchemes] = React.useState('Loading...');
    ...
    <>
      ...
      <SectionMain>
        ...
        >
          ...
          {hasPermission(currentUser, 'READ_APPLICATIONS') && (
            <Link href={'/applications/applications-list'}>
              <div
                className={`${
                  corners !== 'rounded-full' ? corners : 'rounded-3xl'
                } dark:bg-dark-900 ${cardsStyle} dark:border-dark-700 p-6`}
          ...
          {hasPermission(currentUser, 'READ_SCHEMES') && (
